...
void CarregaAlunos(){
    ...
    while(1){
        fread(&alunos[qAlunos], sizeof(alunos[0]), 1, fp);
        if(feof(fp))
            break;
        qAlunos++;
    }
...
void SalvaAlunos(){
    ...
    /* d = fwrite(Disciplina, sizeof(Disciplina[0]), MAX_DISCPLINA, fp); */
    /* for(i = 0; i < qAlunos; i++){
       /* fprintf(fp,"%d\n%s\n%.1f %.1f %.1f %.1f\n", alunos[i].matricula, alunos[i].nomeAluno, alunos[i].nota1, alunos[i].nota2, alunos[i].nota3, alunos[i].notaRecuperacao);
       fwrite(&alunos[i], sizeof(alunos[i]), 1, fp);
    }*/

    i = fwrite(alunos, sizeof(alunos[0]), qAlunos, fp);

    if(fclose(fp) == 0){
        printf("Dados salvos com sucesso!(%d alunos)\n", i);
    }
...
int main(){
    ...
    while (1){
        ...
        default:
        puts("Operacao Invalida, quer quebrar meu codigo??");
            break;
        }
    }
